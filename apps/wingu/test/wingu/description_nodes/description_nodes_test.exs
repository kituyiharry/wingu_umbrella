defmodule Wingu.DescriptionNodesTest do
  ...
  describe "description_nodes" do
    ...
    @update_attrs %{isrequired: false, label: "some updated label", placeholder: "some updated placeholder"}
    ...
    test "create_description_node/1 with valid data creates a description_node" do
      assert {:ok, %DescriptionNode{} = description_node} = DescriptionNodes.create_description_node(@valid_attrs)
    ...
    test "create_description_node/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = DescriptionNodes.create_description_node(@invalid_attrs)
    end

    test "update_description_node/2 with valid data updates the description_node" do
      description_node = description_node_fixture()
      assert {:ok, %DescriptionNode{} = description_node} = DescriptionNodes.update_description_node(description_node, @update_attrs)
    ...
    test "update_description_node/2 with invalid data returns error changeset" do
      description_node = description_node_fixture()
      assert {:error, %Ecto.Changeset{}} = DescriptionNodes.update_description_node(description_node, @invalid_attrs)
    ...
    test "delete_description_node/1 deletes the description_node" do
      description_node = description_node_fixture()
      assert {:ok, %DescriptionNode{}} = DescriptionNodes.delete_description_node(description_node)
      assert_raise Ecto.NoResultsError, fn -> DescriptionNodes.get_description_node!(description_node.id) end
    end
