defmodule WinguWeb.SectionNodeControllerTest do
  ...
  describe "index" do
    test "lists all section_nodes", %{conn: conn} do
      conn = get(conn, Routes.section_node_path(conn, :index))
  ...
  describe "create section_node" do
    test "renders section_node when data is valid", %{conn: conn} do
      conn = post(conn, Routes.section_node_path(conn, :create), section_node: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.section_node_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.section_node_path(conn, :create), section_node: @invalid_attrs)
  ...
  describe "update section_node" do
    ...
    test "renders section_node when data is valid", %{conn: conn, section_node: %SectionNode{id: id} = section_node} do
      conn = put(conn, Routes.section_node_path(conn, :update, section_node), section_node: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, section_node: section_node} do
      conn = put(conn, Routes.section_node_path(conn, :update, section_node), section_node: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete section_node" do
    ...
    test "deletes chosen section_node", %{conn: conn, section_node: section_node} do
      conn = delete(conn, Routes.section_node_path(conn, :delete, section_node))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.section_node_path(conn, :show, section_node))
      end
    end
  end
