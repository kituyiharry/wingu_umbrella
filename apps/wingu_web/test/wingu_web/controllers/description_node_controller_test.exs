defmodule WinguWeb.DescriptionNodeControllerTest do
  ...
  describe "index" do
    test "lists all description_nodes", %{conn: conn} do
      conn = get(conn, Routes.description_node_path(conn, :index))
  ...
  describe "create description_node" do
    test "renders description_node when data is valid", %{conn: conn} do
      conn = post(conn, Routes.description_node_path(conn, :create), description_node: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.description_node_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.description_node_path(conn, :create), description_node: @invalid_attrs)
  ...
  describe "update description_node" do
    ...
    test "renders description_node when data is valid", %{conn: conn, description_node: %DescriptionNode{id: id} = description_node} do
      conn = put(conn, Routes.description_node_path(conn, :update, description_node), description_node: @update_attrs)
  ...
  describe "delete description_node" do
    ...
    test "deletes chosen description_node", %{conn: conn, description_node: description_node} do
      conn = delete(conn, Routes.description_node_path(conn, :delete, description_node))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.description_node_path(conn, :show, description_node))
      end
    end
  end
