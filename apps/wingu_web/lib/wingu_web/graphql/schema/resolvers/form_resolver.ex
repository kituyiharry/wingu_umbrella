defmodule WinguWeb.GraphQL.Resolvers.FormResolver do
  ...
  def create_form(
        _parent,
        %{company: company, form: %{name: n, summary: s, description: d, template: t}},
        %{context: %{"sub" => _id}}
      ) do
    ...
    mul =
      ...
      |> Multi.insert(:form, fn %{get_company: company} ->
        ...
        |> Forms.Form.changeset(%{name: n, summary: s, description: d})
      end)
  ...
  def fill_form(_parent, %{form_id: id, data: sections}, %{context: %{"sub" => sub}}) do
    formdata =
      ...
      |> Multi.insert(:formdata, fn %{:context => %{:form => form, :client => client}} ->
        Ecto.build_assoc(form, :form_data)
      ...
      |> Multi.merge(fn %{:formdata => formdata} ->
        sections
        |> Enum.reduce(Multi.new(), fn section, multi ->
          key = section.section
          section_node = SectionNodes.get_section_node!(section.section)

          Multi.insert(multi, key, fn _change ->
            Ecto.build_assoc(section_node, :section_data)
            |> Ecto.Changeset.change()
            |> Ecto.Changeset.put_assoc(:form_datum, formdata)
          end)
          |> Multi.merge(fn %{^key => section_data} ->
            ...
            |> Enum.reduce(Multi.new(), fn node, multi ->
              ...
              Multi.insert(multi, nest_key, fn _change ->
                desc_node
                |> Ecto.build_assoc(:text_node_data)
  ...
  def is_form_filled(repo, %{context: %{form: form}}, data) when is_list(data) do
    ...
    form_template =
      from(template in FormTemplates.FormTemplate,
        ...
        select: {count(section_node), count(description_node)}
      )
      |> repo.one()

    case Enum.reduce(data, {0, 0}, fn section, {itemcount, subcount} ->
           {itemcount + 1, subcount + length(section.nodes)}
         end) do
      ^form_template ->
        {:ok, form_template}
