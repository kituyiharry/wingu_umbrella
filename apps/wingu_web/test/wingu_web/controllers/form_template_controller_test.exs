defmodule WinguWeb.FormTemplateControllerTest do
  ...
  describe "index" do
    test "lists all form_templates", %{conn: conn} do
      conn = get(conn, Routes.form_template_path(conn, :index))
  ...
  describe "create form_template" do
    test "renders form_template when data is valid", %{conn: conn} do
      conn = post(conn, Routes.form_template_path(conn, :create), form_template: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.form_template_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.form_template_path(conn, :create), form_template: @invalid_attrs)
  ...
  describe "update form_template" do
    ...
    test "renders form_template when data is valid", %{conn: conn, form_template: %FormTemplate{id: id} = form_template} do
      conn = put(conn, Routes.form_template_path(conn, :update, form_template), form_template: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, form_template: form_template} do
      conn = put(conn, Routes.form_template_path(conn, :update, form_template), form_template: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete form_template" do
    ...
    test "deletes chosen form_template", %{conn: conn, form_template: form_template} do
      conn = delete(conn, Routes.form_template_path(conn, :delete, form_template))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.form_template_path(conn, :show, form_template))
      end
    end
  end
